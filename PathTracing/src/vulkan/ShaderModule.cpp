...
ShaderModule ShaderModule::createRayGenShader(VulkanDevice* device,
                                               const std::string& filename,
                                               const std::string& entryPoint) {
    ShaderModuleConfig config;
    config.filename = filename;
    config.stage = ShaderStage::RAY_GEN;
    config.entryPoint = entryPoint;
    return ShaderModule(device, config);
}

ShaderModule ShaderModule::createClosestHitShader(VulkanDevice* device,
                                                   const std::string& filename,
                                                   const std::string& entryPoint) {
    ShaderModuleConfig config;
    config.filename = filename;
    config.stage = ShaderStage::CLOSEST_HIT;
    config.entryPoint = entryPoint;
    return ShaderModule(device, config);
}

ShaderModule ShaderModule::createMissShader(VulkanDevice* device,
                                             const std::string& filename,
                                             const std::string& entryPoint) {
    ShaderModuleConfig config;
    config.filename = filename;
    config.stage = ShaderStage::MISS;
    config.entryPoint = entryPoint;
    return ShaderModule(device, config);
}
