...
VulkanDevice::VulkanDevice(vk::raii::Instance& instance, vk::raii::SurfaceKHR& surface,
    ...
    // Auto-append RT extensions when requireRayTracing is set
    if (config.requireRayTracing) {
        this->config.requiredExtensions.push_back(VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME);
        this->config.requiredExtensions.push_back(VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME);
        this->config.requiredExtensions.push_back(VK_KHR_DEFERRED_HOST_OPERATIONS_EXTENSION_NAME);
    }
...
void VulkanDevice::createLogicalDevice(vk::raii::SurfaceKHR& surface) {
    ...
    // Feature chain: Vulkan 1.1 → 1.2 → accelerationStructure → rayTracingPipeline
    ...
    // Query RT properties if enabled
    if (config.requireRayTracing) {
        auto props2 = physicalDevice.getProperties2<
            vk::PhysicalDeviceProperties2,
            vk::PhysicalDeviceRayTracingPipelinePropertiesKHR,
            vk::PhysicalDeviceAccelerationStructurePropertiesKHR>();
        m_rtPipelineProps = props2.get<vk::PhysicalDeviceRayTracingPipelinePropertiesKHR>();
        m_asProps = props2.get<vk::PhysicalDeviceAccelerationStructurePropertiesKHR>();
        LOG_INFO("Ray tracing pipeline properties queried");
        LOG_INFO("  - ShaderGroupHandleSize: " + std::to_string(m_rtPipelineProps.shaderGroupHandleSize));
        LOG_INFO("  - MaxRecursionDepth: " + std::to_string(m_rtPipelineProps.maxRayRecursionDepth));
        LOG_INFO("  - Min AS build offset alignment: " + std::to_string(m_asProps.minAccelerationStructureScratchOffsetAlignment));
    }
...
int VulkanDevice::rateDeviceSuitability(vk::PhysicalDevice device, vk::SurfaceKHR surface) const {
    ...
    if (deviceFeatures.fillModeNonSolid) score += 25;
